...
var driver = neo4j.driver("bolt://localhost:7687", neo4j.auth.basic("neo4j", "aditya@2003"))
...
//                 return next(err); 
//             } else {
//                 // Check the user type and create the corresponding node in Neo4j
//                 if (req.user.type === 'C_Gvt') { // Central Government
//                     console.log('User type: Central Government');
//                     session
//                         .run("CREATE (n: Central_Gvt {name: $nameParam, balance: 10000}) RETURN n.name", 
//                             { nameParam: req.user.name })
//                         .then(function(result) {
//                             console.log('Central Government node created:', result.records[0].get('n.name'));
//                             return res.redirect('/index');
//                         })
//                         .catch(function(error) {
//                             console.log('Error creating Central Government node:', error);
//                             return res.status(500).send('Error creating node');
//                         });
//                 } else if (req.user.type === 'S_Gvt') { // State Government
//                     console.log('User type: State Government');
//                     session
//                         .run("CREATE (n: State_Gvt {name: $nameParam, balance: 10000}) RETURN n.name", 
//                             { nameParam: req.user.name })
//                         .then(function(result) {
//                             console.log('State Government node created:', result.records[0].get('n.name'));
//                             return res.redirect('/indexDistrict');
//                         })
//                         .catch(function(error) {
//                             console.log('Error creating State Government node:', error);
//                             return res.status(500).send('Error creating node');
//                         });
//                 } else if (req.user.type === 'D_Gvt') { // District Government
//                     console.log('User type: District Government');
//                     session
//                         .run("CREATE (n: District_Gvt {name: $nameParam, balance: 10000}) RETURN n.name", 
//                             { nameParam: req.user.name })
//                         .then(function(result) {
//                             console.log('District Government node created:', result.records[0].get('n.name'));
//                             return res.redirect('/indexSchool');
//                         })
//                         .catch(function(error) {
//                             console.log('Error creating District Government node:', error);
//                             return res.status(500).send('Error creating node');
//                         });
//                 } else if (req.user.type === 'School') { // School
//                     console.log('User type: School');
//                     session
//                         .run("CREATE (n: School {name: $nameParam, balance: 10000}) RETURN n.name", 
//                             { nameParam: req.user.name })
//                         .then(function(result) {
//                             console.log('School node created:', result.records[0].get('n.name'));
//                             return res.redirect('/indexVendor');
//                         })
//                         .catch(function(error) {
//                             console.log('Error creating School node:', error);
//                             return res.status(500).send('Error creating node');
//                         });
//                 }
//             }
        ...
        req.logIn(user, function(err) {
            if (err) { 
                ...
                if (req.user.type === 'C_Gvt') { // Central Government
                    console.log('User  type: Central Government');
                    session
                        .run("MERGE (n: Central_Gvt {name: $nameParam}) ON CREATE SET n.balance = 10000 RETURN n.name", 
                            { nameParam: req.user.name })
                        .then(function(result) {
                            console.log('Central Government node created:', result.records[0].get('n.name'));
                            return res.redirect('/index');
                    ...
                    console.log('User  type: State Government');
                    session
                        .run("MERGE (n: State_Gvt {name: $nameParam}) ON CREATE SET n.balance = 10000 RETURN n.name", 
                            { nameParam: req.user.name })
                        .then(function(result) {
                            console.log('State Government node created:', result.records[0].get('n.name'));
                            return res.redirect('/indexDistrict');
                    ...
                    console.log('User  type: District Government');
                    session
                        .run("MERGE (n: District_Gvt {name: $nameParam}) ON CREATE SET n.balance = 10000 RETURN n.name", 
                            { nameParam: req.user.name })
                        .then(function(result) {
                            console.log('District Government node created:', result.records[0].get('n.name'));
                            return res.redirect('/indexSchool');
                    ...
                    console.log('User  type: School');
                    session
                        .run("MERGE (n: School {name: $nameParam}) ON CREATE SET n.balance = 10000 RETURN n.name", 
                            { nameParam: req.user.name })
                        .then(function(result) {
                            console.log('School node created:', result.records[0].get('n.name'));
                            return res.redirect('/indexVendor');
...
app.post('/central/add', checkAuthenticated, function(req, res) {
    const name = req.body.name;
    const password = uuid().split('-').join('').substr(0, 8); // Generate a random password
    ...
    if (!found) {
        users.push({
            ...
            type: 'C_Gvt', // Set user type for Central Government
        });

        // Create a new user in the blockchain (if needed)
        blockchain.createNewUser(name, 0, 'C_Gvt');
    }

    console.log(users);

    // Create Central Government node in Neo4j
    session
        .run("CREATE (n: Central_Gvt {name: $nameParam, balance: 0}) RETURN n.name", { nameParam: name })
        .then(function(result) {
            console.log('Central Government node created:', result.records[0].get('n.name'));
            res.redirect('/index');
        })
...
app.post('/school/add', checkAuthenticated, function(req, res){
    ...
    session
        .run("MERGE (n: School {name: $nameParam}) ON CREATE SET n.balance = 0 RETURN n.name", { nameParam: name })
        .then(function(result){
            var mailOptions = {
                from: 'test.sparrow.8688@gmail.com',
                to: email,
...
app.post('/allocate/ctos', checkAuthenticated, async function(req, res) {
    try {
        const { name1, name2, amount, project } = req.body;
        
        // Fetch sender's data
        const senderData = blockchain.getUserDataByName(name1);
        if (!senderData) {
        ...
        const newTrans = blockchain.createNewTransaction(amount, name1, name2, project);
        ...
        console.log('New Block:', blockchain.getLastBlock());

        // Update user balances
        blockchain.updateUserSender(name1, amount);
        blockchain.updateUserReceiver(name2, amount);

        const updatedSender = blockchain.getUserDataByName(name1);
        const updatedReceiver = blockchain.getUserDataByName(name2);

        // Update balances in the Neo4j database
        await session.run("MATCH (a:Central_Gvt {name: $nameParam1}) SET a.balance = $senderBalance", {
            nameParam1: name1,
            senderBalance: updatedSender.balance
        });

        await session.run("MATCH (b:State_Gvt {name: $nameParam2}) SET b.balance = $receiverBalance", {
            nameParam2: name2,
            receiverBalance: updatedReceiver.balance
        });

        // Create a relationship in the Neo4j database
        await session.run("MATCH (a:Central_Gvt {name: $nameParam1}), (b:State_Gvt {name: $nameParam2}) " +
            "MERGE (a)-[r:" + project + " {amount: $amountPara}]->(b) RETURN a, b", {
                nameParam1: name1,
                nameParam2: name2,
                amountPara: amount
            });
